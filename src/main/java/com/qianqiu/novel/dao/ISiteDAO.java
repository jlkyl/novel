...
public interface ISiteDAO {
    ...
    @Insert("insert into site values (null,#{sitename},#{siteurl},#{operateeid},now())")
    Integer add(Site site);
    @Select("select *,getEmp(operateeid) empname from site")
    List<Site> findAll();
    @Update("update site set sitename=#{sitename},siteurl=#{siteurl},operateeid=#{operateeid},operatedate=now() where siteid=#{siteid}")
